...
2. email()
    ...
    同时，Worker 实现了 `email()` 处理器并声明 `EMAIL_HANDLER` 路由绑定。

    部署后可在 **Compute > Email Service > Email Routing > Routing Rules** 创建规则，

    选择 **Send to a Worker**，然后在 Worker 下拉框中选择此 Worker；收到的邮件会转发至 `EMAIL_TO`。
